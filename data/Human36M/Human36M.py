...
class Human36M(torch.utils.data.Dataset):
    ...
    def load_data(self, video_frame_num=16):
        ...
        for aid in db.anns.keys():
            ...
            if len(video_split) == video_frame_num:
                if len(set([obj['img_path'].split('/')[0] for obj in video_split])) == 1:
        ...
        for data in data_video_split:
            ...
            if self.data_split == 'train':
                ...
                targets = {'joint_img': joint_img, 'smplx_joint_img': smplx_joint_img, 
                        ...
                        'smplx_cam_trans': smplx_cam_trans, # self-add
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        sample_num = len(outs)
        eval_result = {'mpjpe': [], 'pa_mpjpe': [], 'accel_body': []}
        for n in range(sample_num):
            video_annot = annots[cur_sample_idx + n]
            video_out = outs[n]
            video_mpjpe = []
            video_pa_mpjpe = []
            out_video_body_coords = []
            for annot, out_smplx_mesh_cam in zip(video_annot, video_out['smplx_mesh_cam']):
                ...
                out_video_body_coords.append(joint_out[np.newaxis,:,:])
            ...
            out_video_body_coords = np.concatenate(out_video_body_coords, axis=0)

            eval_result['mpjpe'].append(np.array(video_mpjpe).mean())
            eval_result['pa_mpjpe'].append(np.array(video_pa_mpjpe).mean())

            eval_result['accel_body'].append(compute_accel(out_video_body_coords).mean())
    ...
    def print_eval_result(self, eval_result):
        ...
        print('ACCEL (Body): %.2f mm/s²' % np.mean(eval_result['accel_body']))
...
def compute_accel(joints):
    ...
    # return np.mean(acceleration, axis=1)
